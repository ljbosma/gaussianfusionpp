...
class Debugger(object):
  ...
  def plot_radar2img(self, img_id, img, batch, hmap_str='depth'): 
      ...
      if pc_ef.ndim == 2:
        ...
        raise ValueError(f"Unexpected pc_ef shape: {pc_ef.shape}")
  ...
  def add_bird_views(self, dets_dt, dets_gt, vis_thresh=0.3, img_id='bird', 
                   pc_3d=None, draw_ego=True, show_velocity=False, max_range=None):
    # Dynamically set world size
    if max_range is not None:
        self.world_size = int(np.ceil(max_range + 10))  # add 10m margin
            ...
            if item['score'] > vis_thresh \
                ...
                for e in [[0, 1]]:
                    ...
                    if pt1 is not None and pt2 is not None:
                        ...
                        cv2.line(bird_view, pt1, pt2, lc, 2, lineType=cv2.LINE_AA)

                if show_velocity and 'velocity' in item:
                    str_pt = (int((rect[0][0] + rect[1][0]) / 2), int((rect[0][1] + rect[1][1]) / 2))
                    end_pt = (int(str_pt[0] + 5 * item['velocity'][0]), 
                              int(str_pt[1] - 5 * item['velocity'][2]))
    ...
    # === Annotate max_range on the BEV ===
    if max_range is not None:
        cv2.putText(bird_view, f"BEV max range: {max_range:.1f} m", 
