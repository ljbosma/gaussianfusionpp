...
def compute_max_ground_truth_range(gt_dets):
  max_range = 0
  for item in gt_dets:
    if 'loc' in item:
      loc = np.array(item['loc'])
      dist = np.linalg.norm(loc[[0, 2]])  # xz plane
...
class ModelWithLoss(torch.nn.Module):
  ...
  def forward(self, batch, phase, eval_frustum=None):
    ...
    # print("pc_lfa_feat requires_grad:", outputs[-1]['pc_lfa_feat'].requires_grad)
    # print("Model outputs keys:", outputs[-1].keys())

    
    # # Log raw outputs from the model
    # if self.opt.debug > 0:
    #   for key, value in outputs[-1].items():
    #     if torch.isnan(value).any():
    #       print(f"NaN detected in output '{key}' before processing")

    # # Process outputs (sigmoid, etc.)
    # processed_outputs = [self.loss._sigmoid_output(output) for output in outputs]
    ...
    #       if torch.isnan(value).any():
    #         print(f"NaN detected in output '{key}' after sigmoid in stack {i}")
  ...
  def run_epoch(self, phase, epoch, data_loader, eval_frustum=None):
    ...
    for iter_id, batch in enumerate(data_loader):
      ...
      if not torch.isfinite(loss):
          print("\n🚨 Loss went NaN! Debug info:")
          print(f"  iter_id: {iter_id}")
          if 'meta' in batch and isinstance(batch['meta'], list) and 'img_id' in batch['meta'][0]:
              print(f"  image_id: {batch['meta'][0]['img_id']}")
          for k, v in loss_stats.items():
              print(f"  {k}: {v.item() if v is not None and torch.is_tensor(v) else v}")
          print("Exiting to debug.")
          exit(1)
  ...
  def debug(self, batch, output, iter_id, dataset):
    ...
    for i in range(1):
      ...
      if 'rot' in opt.heads and 'dim' in opt.heads and 'dep' in opt.heads:
        ...
        max_range = compute_max_ground_truth_range(det_gt[i])
